...
const getFeeIcon = (title, type) => {
  ...
  if (titleLower.includes("dinner") || titleLower.includes("party") || titleLower.includes("event")) {
    return { icon: <PartyPopper className="w-6 h-6 text-purple-600" />, bg: "bg-purple-100" };
  }
  if (titleLower.includes("shirt") || titleLower.includes("merchandise") || titleLower.includes("cloth")) {
    return { icon: <Shirt className="w-6 h-6 text-green-600" />, bg: "bg-green-100" };
  }
  if (titleLower.includes("ticket")) {
    return { icon: <Ticket className="w-6 h-6 text-orange-600" />, bg: "bg-orange-100" };
  }
    ...
    return (
      <div className="mb-8">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-8 text-center">
          ...
          <p className="text-sm text-gray-500">You have no pending fees at the moment.</p>
        </div>
      </div>
    );
  }
