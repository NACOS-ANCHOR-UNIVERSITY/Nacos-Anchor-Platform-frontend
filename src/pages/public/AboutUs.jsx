  ...
  const coreMission = [
    {
      ...
      description: "Creating a unified network where students from 100L to 400L can connect, share resources, and foster peer-to-peer learning environments."
    },
    {
      icon: Commnication,
      heading: "Communication",
      description: "Bridging the information gap between the department faculty and the student body through transparent and timely digital channels."
    },
    {
      icon: Innovation,
      heading: "Innovation",
      description: "Encouraging students to build real-world solutions. We support hackathons, workshops, and project showcases that solve local problems."
    }
  ]
  ...
  const timelineData = [
    {
      ...
      description: "NACOS Anchor University chapter was officially inaugurated, setting the foundation for student-led technological growth on campus.",
      icon: flag
    },
    {
      year: "2020",
      title: "Inaugural Tech Week",
      description: "Despite global challenges, we hosted our first hybrid Tech Week, featuring guest speakers from major Nigerian tech firms.",
      icon: calender
    },
    {
      year: "2023",
      title: "Digital Platform Launch",
      description: "The launch of this centralized portal to streamline dues, event registration, and academic resources for all students.",
      icon: grid
    }
  ]
  ...
  return (
    <div className="py-20 px-4 sm:px-8 md:px-18 lg:px-30 min-h-screen w-full flex flex-col justify-center items-center gap-20 bg-[#F6F8F5]">
      ...
      <section className="flex flex-col items-center">
        <h1 className="text-[#0F172A] text-[24px] md:text-[36px] font-bold">Our Core Mission</h1>
        <p
          className="text-sm sm:text-[18px] font-normal text-[#475569] text-center max-w-2xl"
        >
          Driven by a commitment to excellence, student welfare, and technological advancement within
        ...
        <div className="flex flex-col lg:flex-row gap-6 mt-10 w-full">
          {coreMission.map(({ icon, heading, description }, index) => (
            <div className="bg-white rounded-2xl border-[#E2E8F0] border-2 p-6 flex-1" key={index}>
              ...
              <div>
                <h2 className="text-[#0F172A] text-[18px] md:text-[20px] font-bold mt-4">{heading}</h2>
                <p className="text-[#475569] text-sm md:text-base font-normal mt-2">{description}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="flex flex-col items-center justify-center w-full max-w-3xl mt-10">
        ...
        <div className="relative mt-10 w-full">
          ...
          <div className="flex flex-col gap-10">
            {timelineData.map(({ year, title, description, icon }, index) => (
      ...
      <section className="flex flex-col items-center mt-10">
        <h1 className="text-[#0F172A] text-[24px] md:text-[36px] font-bold">Meet the Executives</h1>
        <p className="text-sm sm:text-[18px] font-normal text-[#475569] text-center max-w-2xl mt-2">The team working behind the scenes to serve the department.</p>
