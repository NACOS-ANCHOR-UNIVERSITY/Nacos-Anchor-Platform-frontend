...
const Events = () => {
  ...
  const getButtonIcon = (buttonText) => {
    if (buttonText.toLowerCase().includes("ticket"))
    ...
    <div className="py-20 px-4 sm:px-8 md:px-20 lg:px-32 min-h-screen w-full flex flex-col items-center gap-20 bg-[#F6F8F5]">
      ...
      <div className="flex justify-center items-center rounded-2xl relative w-full h-64 md:h-80 lg:h-96 overflow-hidden group bg-gradient-to-r from-green-900 to-green-700">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white flex flex-col items-center w-full max-w-2xl px-4 text-center">
          ...
          <p className="text-sm md:text-lg font-normal max-w-xl">
            Join workshops, hackathons, and social gatherings designed to boost
            your tech career and network at NACOS Anchor University.
          </p>
        </div>
      </div>
      ...
      <div className="w-full">
        ...
        <div className="flex flex-col gap-8 mb-10">
          <div className="flex flex-col gap-6 md:flex-row justify-between md:items-center mb-6">
            <div className="flex">
              <button className="text-[14px] font-bold border-b-2 border-[#138301] px-4 pb-2.5">
                Upcoming Events
              </button>
              <button className="text-[14px] font-bold border-b-2 border-[#D0E6CC] px-4 pb-2.5 text-gray-500">
                Past Events
              </button>
            </div>
            <input
          ...
          <div className="flex flex-wrap gap-3">
            {["All Events", "Workshop", "Hackathon", "Social", "Seminar"].map(
              (item, index) => (
                <button
                  key={index}
                  onClick={() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {currentItems.map((event) => {
              ...
              return (
                <div
                  key={event.id}
                  className="bg-white rounded-2xl p-4 border border-[#E2E8F0] hover:shadow-xl transition-all group flex flex-col h-full relative overflow-hidden"
                >
                  {/* Image Container */}
                  ...
                  <div className="flex flex-col grow gap-2">
                    <h3 className="text-xl font-bold text-[#0F172A] line-clamp-2">
                      {event.title}
                    </h3>
                    ...
                    <button
                      ...
                      className={`w-full py-3 rounded-lg font-medium transition-colors mt-auto flex items-center justify-center gap-2 ${isSoldOut ? "bg-gray-100 text-gray-400 cursor-not-allowed" : "bg-[#E8F3E6] text-[#138301] hover:bg-[#D0E6CC]"}`}
                    >
                      {event.button_text}
        ...
        {!loading && !error && filteredEvents.length > itemsPerPage && (
          <div className="flex justify-center items-center gap-2 mt-12">
            <button
              ...
              className={`w-10 h-10 rounded-lg border flex items-center justify-center transition-colors ${currentPage === 1 ? "border-gray-200 text-gray-300 cursor-not-allowed" : "border-[#D0E6CC] text-[#0F172A] hover:bg-[#E8F3E6]"}`}
            >
              &lt;
            </button>
            ...
            {Array.from({ length: totalPages }, (_, i) => (
              <button
                ...
                className={`w-10 h-10 rounded-lg font-bold border transition-all ${currentPage === i + 1 ? "bg-[#138301] text-white border-[#138301]" : "bg-white text-[#0F172A] border-[#D0E6CC] hover:bg-[#E8F3E6]"}`}
              >
                {i + 1}
              </button>
            ))}
            ...
            <button
              ...
              className={`w-10 h-10 rounded-lg border flex items-center justify-center transition-colors ${currentPage === totalPages ? "border-gray-200 text-gray-300 cursor-not-allowed" : "border-[#D0E6CC] text-[#0F172A] hover:bg-[#E8F3E6]"}`}
            >
              &gt;
            </button>
          </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 justify-between items-center bg-[#E8F4E6] border border-[#D0E6CC] rounded-2xl p-8 md:p-12 w-full">
        <div className="max-w-xl">
          <h3 className="font-bold text-2xl md:text-3xl text-[#0F1C0C]">
          ...
          <p className="text-sm md:text-base text-[#475569] mt-3 leading-relaxed">
            ...
            workshops, exam schedules, and department news.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          <input
            ...
            className="bg-white border border-[#D0E6CC] rounded-lg px-4 py-3 outline-none w-full sm:w-64 focus:border-[#138301] transition-colors"
          />
